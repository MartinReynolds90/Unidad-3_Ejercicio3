...
void Game::InitPhysics()
{
    ...
    controlBody_fijo = Box2DHelper::CreateRectangularStaticBody(phyWorld, 10, 10);//TAMAÑO DEL CUBO
    controlBody_fijo->SetTransform(b2Vec2(20.0f, 30.0f), 0.0f);//POSICION-ANGULO DELCUBO FIJO
    ...
    jointDef.Initialize(controlBody, controlBody_fijo, b2Vec2(50.0f, 50.0f), b2Vec2(20.0f, 30.0f));
    jointDef.collideConnected = true;                       //POS_PELOTA------POS CUBO FIJO
    ...
    MouseDef.bodyA = controlBody_fijo;
    MouseDef.bodyB = controlBody;
    MouseDef.target.Set(0.0f, 10.0f);  //b2Vec2(sf::Mouse::getPosition().x, sf::Mouse::getPosition().y);
    MouseDef.maxForce = 1000.0f;
    MouseDef.damping = 0.5;

    b2MouseJoint* mouseDef = (b2MouseJoint*)phyWorld->CreateJoint(&MouseDef);
